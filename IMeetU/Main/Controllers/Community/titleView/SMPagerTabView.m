...
- (void)buildUI {
    ...
    for (int i = 0; i < number; i++) {
        ...
        [self.arrBtn addObject:itemButton];
...
- (UIView *)selectedLine {
    if (!_selectedLine) {
        self.selectedLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.tabView.height - 1.5, self.selectedLineWidth, 2.0)];
...
-(void)setHide{
    for (int i = 0; i<self.arrBtn.count; i++) {
        UIButton * btn =[self.arrBtn objectAtIndex:i];
        btn.hidden=YES;
    }
