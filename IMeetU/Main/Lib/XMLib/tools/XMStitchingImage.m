...
@implementation XMStitchingImage
...
- (UIImage*)stitchingImages:(NSArray*)images size:(CGSize)size margin:(CGFloat)margin backgroundColor:(UIColor*)color{
    ...
    UIImageView *v = [self stitchingOnImageView:imgView withImageViews:imageViews marginValue:margin];
    
    UIGraphicsBeginImageContext(v.bounds.size);     //currentView 当前的view  创建一个基于位图的图形上下文并指定大小为
    [v.layer renderInContext:UIGraphicsGetCurrentContext()];//renderInContext呈现接受者及其子范围到指定的上下文
