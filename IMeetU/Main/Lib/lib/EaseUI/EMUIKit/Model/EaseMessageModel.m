...
#pragma mark - 表情MM修改
        ...
        NSString *mmExtObj = ext[@"mm_ext"];
        if (mmExtObj != nil) {
            if ([mmExtObj isKindOfClass:[NSDictionary class]]) {
                self.mmExt = (NSDictionary*)mmExtObj;
            }else{
                NSData *extData = [mmExtObj dataUsingEncoding:NSUTF8StringEncoding];
                self.mmExt = [NSJSONSerialization JSONObjectWithData:extData options:NSJSONReadingMutableLeaves error:nil];
            }
        }
