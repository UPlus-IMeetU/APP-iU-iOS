...
@property (nonatomic, strong) UILabel *titleNavigation;
...
- (void)viewDidLoad
{
    ...
    [self.viewNavigation addSubview:self.btnBack];
    [self.viewNavigation addSubview:self.btnFinish];
    [self.viewNavigation addSubview:self.titleNavigation];
    
    _mapView = [[MKMapView alloc] initWithFrame:CGRectMake(0, 64, [UIScreen screenWidth], [UIScreen screenHeight])];
...
#pragma mark - 自定义方法
#pragma mark 返回
- (void)onClickBtnBack:(UIButton*)sender{
    [self.navigationController popViewControllerAnimated:YES];
}
#pragma mark 发送地址
- (void)onClickBtnFinish:(UIButton*)sender{
    if (_delegate && [_delegate respondsToSelector:@selector(sendLocationLatitude:longitude:andAddress:)]) {
        [_delegate sendLocationLatitude:_currentLocationCoordinate.latitude longitude:_currentLocationCoordinate.longitude andAddress:_addressString];
    }
...
- (UIView *)viewNavigation{
    if (!_viewNavigation) {
        _viewNavigation = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen screenHeight], 64)];
        _viewNavigation.backgroundColor = [UIColor often_6CD1C9:1];
    }
...
- (UIButton *)btnBack{
    if (!_btnBack) {
        _btnBack = [[UIButton alloc] initWithFrame:CGRectMake(0, 20, 54, 44)];
        ...
        [_btnBack addTarget:self action:@selector(onClickBtnBack:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _btnBack;
}
...
- (UIButton *)btnFinish{
    if (!_btnFinish) {
        _btnFinish = [[UIButton alloc] initWithFrame:CGRectMake([UIScreen screenWidth]-54, 20, 54, 44)];
        [_btnFinish setTitle:@"发送" forState:  UIControlStateNormal];
        [_btnFinish setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_btnFinish addTarget:self action:@selector(onClickBtnFinish:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _btnFinish;
}

- (UILabel *)titleNavigation{
    if (!_titleNavigation) {
        _titleNavigation = [[UILabel alloc] initWithFrame:CGRectMake(54, 20, [UIScreen screenWidth]-108, 44)];
        _titleNavigation.font = [UIFont systemFontOfSize:16];
        _titleNavigation.textColor = [UIColor whiteColor];
        _titleNavigation.textAlignment = NSTextAlignmentCenter;
        _titleNavigation.text = @"当前位置";
    }
    return _titleNavigation;
}
