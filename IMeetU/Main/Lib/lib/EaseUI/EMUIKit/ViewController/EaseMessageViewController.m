        ...
        for (id obj in textImgArray) {
            if ([obj isKindOfClass:[MMEmoji class]]) {
                ...
                if (!mmExt) {
                    ...
                    NSData  *jsonData = [NSJSONSerialization dataWithJSONObject:mmExt
                                                                        options:NSJSONWritingPrettyPrinted error:&parseError];
                    NSString *mmExtString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
                    
                    ext = @{@"mm_ext":mmExtString};
                }
...
-(void)sendMMFaceMessage:(MMEmoji *)emoji
{
    ...
    NSDictionary *ext = @{@"mm_ext":mmExtString};
    [self sendMMFaceMessage:emoji withExt:ext];
}
